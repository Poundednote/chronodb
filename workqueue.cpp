...
{
	while (wq->producer_count.load(std::memory_order::acquire) != 0 ||
	       wq->completion_count.load(std::memory_order::acquire) !=
		       wq->tail.load(std::memory_order::relaxed)) {
		cpu_pause();
	}
}
...
void mpmc_work_queue_stop(MPMCWorkQueue *wq)
{
        ...
        wq->tail.store(0xFFFFFFFFFFFFFFFF, std::memory_order::relaxed);
        wq->tail.notify_all();
}
...
void mpsc_writer_enqueue(MPSCWriterQueue *wq, MPSCWriterQueueEntry entry)
{
	...
	for (;;) {
		if (wq->tail.compare_exchange_weak(tail, next_tail_index, std::memory_order::acquire, std::memory_order::relaxed)) {
